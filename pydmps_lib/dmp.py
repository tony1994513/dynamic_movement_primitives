...
class DMPs(object):
    ...
    def imitate_path(self, y_des,num=None, plot=False):
        ...
        if plot is True:
            ...
            plt.figure(num,facecolor='w', edgecolor='w')
            plt.subplot(211)
            psi_track = self.gen_psi(self.cs.rollout())
            plt.plot(psi_track,lw=2)
            plt.title('basis functions')

            # plot the desired forcing function vs approx
            plt.subplot(212)
            plt.plot(f_target[:,0],lw=2)
            plt.plot(np.sum(psi_track * self.w[0], axis=1) * self.dt,lw=2)
